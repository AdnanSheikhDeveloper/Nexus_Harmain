...
export default function HomePage() {
  ...
  return (
    <>
      {/* 1. Hero */}
      <PageHero
        image={images.heroMakkah}
        alt="Aerial view of Makkah and Masjid al-Haram at golden hour"
        size="large"
      >
        <p className="section-overtitle mb-6">{h.heroOvertitle}</p>
        <h1 className="section-heading mb-6 max-w-4xl text-4xl font-bold leading-tight sm:text-5xl lg:text-6xl">
          {h.heroTitle}{" "}
      ...
      <section className="section-dark bg-[#030f26] py-24">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          ...
          <HorizontalScroll className="mobile-service-scroll md:grid-cols-2 lg:grid-cols-3">
            {h.whatWeDo.services.map((service, i) => (
              <Link
                key={service.title}
                href={serviceHrefs[i]}
                className="mobile-service-card card-equal group w-[300px] max-w-[300px] shrink-0 snap-center overflow-hidden rounded-2xl border border-white/[0.08] md:w-auto md:max-w-none md:min-w-0 md:shrink md:h-auto md:min-h-0 md:max-h-none"
              >
                <div className="relative h-52 shrink-0">
                  ...
                  />
                  <div className="absolute inset-0 bg-gradient-to-t from-[#081736] via-[#030f26]/25 to-transparent" />
                </div>
      ...
      <section className="section-dark bg-[#030f26] py-24">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          ...
          <HorizontalScroll className="mobile-why-scroll md:grid-cols-2 lg:grid-cols-3">
            {h.whyUs.items.map((item, i) => {
              const Icon = whyIcons[i];
              return (
                <div
                  key={item.title}
                  className="mobile-why-card card-equal w-[280px] max-w-[280px] shrink-0 snap-center rounded-2xl border border-white/[0.08] bg-[#081736] p-8 transition-colors hover:border-[#c5a880]/35 md:w-auto md:max-w-none md:min-w-0 md:shrink md:h-auto md:min-h-0 md:max-h-none"
                >
                  <Icon className="mb-4 h-8 w-8 shrink-0 text-[#c5a880]" />
      ...
      <section className="bg-[#f4f6f9] py-20 sm:py-24">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          ...
          >
            ...
            <h2 className="section-heading mb-5 text-3xl font-bold leading-tight sm:text-4xl lg:text-[2.75rem]">
              {h.partnership.title}
            </h2>
            <p className="mb-8 max-w-lg text-base leading-relaxed sm:text-lg">
